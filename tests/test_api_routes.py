...
@pytest.mark.parametrize(
    ...
    ids=[
        "validate_by_id",
        "validate_with_missing_profile_name",
        "validate_with_missing_webhook_url",
        "validate_with_missing_root_path",
        "validate_with_missing_root_path_and_profile_name_and_webhook_url",
    ],
)
def test_validate_by_id_success(
    minio_client: FlaskClient,
    crate_id: str,
    payload: dict,
    profiles_path: str,
    status_code: int,
    ...
    with patch(
        ...
        response = minio_client.post(
            f"/v1/ro_crates/{crate_id}/validation", json=payload
        )
...
@pytest.mark.parametrize(
    ...
    ids=["missing_crate_id_returns_404", "missing_minio_bucket_returns_422"],
)
def test_validate_fails_missing_elements(
    minio_client: FlaskClient, crate_id: str, payload: dict, status_code: int
):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            422,
            "Required parameter crate_json is empty",
        ),
    ],
    ids=[
        "failure_missing_crate",
        "failure_empty_crate",
        "failure_malformed_crate",
        "failure_empty_crate",
    ],
)
def test_validate_metadata_failure(
    client: FlaskClient, payload: dict, status_code: int, response_text: str
):
    response = client.post("/v1/ro_crates/validate_metadata", json=payload)
    assert response.status_code == status_code
...
def test_get_validation_by_id_failures(
    minio_client: FlaskClient, crate_id: str, payload: dict, status_code: int
):
...
def test_get_validation_by_id_success(minio_client):
    ...
    with patch(
        ...
        mock_get.assert_called_once_with(
            payload["minio_config"], "crate-123", "base_path"
        )
